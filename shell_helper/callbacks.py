...
def ClearCallback(scope, name, callback):
    """
    Removes a specific callback function from the named callback queue
    """
    try:
        # The queue is like a FIFO in that we can only read it in the order it was written.
        # To remove an entry we must read all the entries and copy them to a new queue (except
        # for the one we wish to remove).
        callback_map = _GetCallbackMap(scope)
        callback_queue = callback_map[name]
        saved_callbacks = PriorityQueue()
        for entry in callback_queue:
            if entry.callback != callback:
                saved_callbacks.put(entry, priority = entry.priority)
        callback_map[name] = saved_callbacks
    except KeyError:
        pass  # there were no registered callbacks under this name
